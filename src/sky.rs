...
impl Sky {
    ...
    pub fn red_sunset2() -> Sky {
        Sky {
            depolarization_factor: 0.02,
            mie_coefficient: 0.008,
            mie_directional_g: 0.62,
            mie_k_coefficient: vec3(0.686, 0.67, 0.4),
            mie_v: 3.936,
            mie_zenith_length: 34000.0,
    ...
    pub fn render(&self, dir: Vec3, sun_position: Vec3) -> Vec3 {
        ...
        let zenith_angle = (0.0f32.max(Vec3::Y.dot(dir))).acos();
        let denom =
        ...
        let sun_e = self.sun_intensity(sun_direction.dot(Vec3::Y));
        let mut lin =
        ...
        lin *= Vec3::splat(1.0).lerp(
            ...
            (1.0 - Vec3::Y.dot(sun_direction)).powf(5.0).clamp(0.0, 1.0),
        );
        ...
        let low_falloff = (Vec3::Y.dot(dir) + 0.4).powf(5.0).max(0.0);
        color = (color * 0.1).powf(3.0) * low_falloff;
        color.powf(1.0 / (1.2 + (1.2 * sunfade))) * 0.5
    }

    fn total_rayleigh(&self, lambda: Vec3) -> Vec3 {
        (8.0 * PI.powi(3)
    ...
    fn total_mie(&self, lambda: Vec3) -> Vec3 {
